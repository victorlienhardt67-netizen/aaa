...
async function analyzeBriefWithClaude(params: AnalyzeBriefParams): Promise<ProductionPlan> {
  ...
  const res = await fetch("/api/claude/analyze-brief", {
    ...
    body: JSON.stringify({
      ...
      stylePositivePrompt: `${style.photoPrompt} ${style.videoPrompt}`,
      lang,
      targetDuration,
      motionIntensity,
      learningContext,
...
export async function extractStyleFromImages(
  ...
  return {
    ...
    photoPrompt: data.positivePrompt,
    videoPrompt: "Smooth natural motion consistent with this visual style, subtle camera movement.",
